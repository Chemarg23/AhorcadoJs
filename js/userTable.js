$(document).ready(() => {
  removeContent();
  $(".list-words").click(listWords);
  $(".list-users").click(listUsers);
  $(".add-word").click(showAddWord);
  $(".show-perfil").click(showPerfil);
  $("#add").click(addWord);
  $(".add-category").click(showAddCategory);
...
function populateTable(table, data, rowHtmlCallback) {
  ...
  $("#miTabla_filter input").attr("placeholder", "Buscar...");
  $('#miTabla_filter label[for="miTabla_filter"]').contents().unwrap();
...
function changePass() {
  ...
  if (currentPassword === "" || newPassword === "" || confirmPassword === "") {
    ...
    $("#errorMessages").html(
      '<i class="bi bi-exclamation-circle p-2"></i><p>Todos los campos son obligatorios.</p>'
    );
  } else if (newPassword !== confirmPassword) {
    $("#confirmpass").addClass("is-invalid");
    $("#errorMessages").html(
      '<i class="bi bi-exclamation-circle p-2"></i><p>Las nuevas contraseñas no coinciden.</p>'
    );
  } else {
    const formData = new FormData();
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.success == false) {
          ...
          $("#errorMessages").html(
            '<i class="bi bi-exclamation-circle p-2"></i><p>La contraseña es incorrecta</p>'
          );
        }
        console.log(data);
      },
